...
/*   Updated: 2025/06/10 22:04:55 by adammour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mlx(t_game *game)
{
	...
	if (!game->mlx_ptr)
		return (free_tab(game->map), write(2,
	...
	if (!game->window_ptr)
		return (free_tab(game->map), free(game->mlx_ptr), write(2,
...
int	init_img(t_game *game)
{
	...
	game->img_exit = mlx_xpm_file_to_image(game->mlx_ptr, "img/exit.xpm",
			&img_size, &img_size);
	game->img_collect = mlx_xpm_file_to_image(game->mlx_ptr, "img/coin.xpm",
			&img_size, &img_size);
	if (!game->img_wall || !game->img_exit || !game->img_floor
		|| !game->img_player || !game->img_collect)
